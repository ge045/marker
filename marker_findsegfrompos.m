function rows = marker_findsegfrompos(drawerobj, xi1)
% function rows = marker_findsegfrompos(drawerobj, xi1)
%
% Find a label segment from its x-axis position. xi1 must be prepared from
% a mouse pointer return, e.g. by 
...
rows = find((drawerobj.seglist(:,1)<=xi1) & (drawerobj.seglist(:,2)>=xi1));
